...
void Reset_Handler(void) {
	...
	/* Reset the RCC clock configuration to the default reset state *///why?, what are the states of registers after reset
	/* Set HSION bit */
	RCC->CR |= (uint32_t)0x00000001;
	/* Reset SW, HPRE, PPRE1, PPRE2, ADCPRE and MCO bits */
	RCC->CFGR &= (uint32_t)0xF8FF0000;
	/* Reset HSEON, CSSON and PLLON bits */
	RCC->CR &= (uint32_t)0xFEF6FFFF;//whats with these casts?
	/* Reset HSEBYP bit */
	...
	/* Configure the System clock frequency, HCLK, PCLK2, ADC and PCLK1 prescalers */
...
void (* const g_pfnVectors[])(void) = {
		(intfunc)&_estack, /* The stack pointer after relocation */
